...
internal class PostgreTransactionsRepository : ITransactionsRepository
{
 ...
 {
 var sql = $@"SELECT id, date, sender_id AS ""SenderId"", recipient_id AS ""RecipientId"", employee_id AS ""EmployeeId"", amount, description
FROM {TableName}
WHERE id = @Id";
...
ORDER BY date";
 ...
 {
 var sql = $@"INSERT INTO {TableName} (date, sender_id, recipient_id, employee_id, amount, description)
